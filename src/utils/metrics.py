...
# Optional metrics requiring CPU
try:
...
def align_signals(ref: torch.Tensor, est: torch.Tensor):
    """Truncate both tensors to same length. Works on GPU."""
    ref = ref.squeeze()
    est = est.squeeze()
...
def snr_db(ref: torch.Tensor, est: torch.Tensor, use_alignment: bool = True) -> float:
    ...
    If use_alignment is True, we first time-align est to ref using align_signals().
    This is important with neural vocoders, which may introduce small delays.
    ...
    if use_alignment:
        # align_signals is already defined in your metrics.py for STOI/PESQ
    ...
    # Ensure same length
    min_len = min(ref_aligned.numel(), est_aligned.numel())
...
def compute_stoi_pesq(
    ...
    # --- Resample to target_sr (e.g., 16000) ---
    try:
