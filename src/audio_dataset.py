...
class AudioSRDataset(Dataset):
    """
    Dataset that creates (low-res mel, high-res mel) pairs from WAV/FLAC files.
    ...
    The filenames in hr/ and low_sr/ must match.
    """
    ...
    def _random_crop(self, wav_hr: torch.Tensor, wav_low: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Random synchronized crop to fixed length."""
        _, T_hr = wav_hr.shape
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Compute log-mels
        mel_hr = self.mel_transform(wav_hr)  # [1, 1, n_mels, T]
        mel_low = self.mel_transform(wav_low)

        # We want per-sample shape [1, n_mels, T] so batch is [B, 1, n_mels, T]
        mel_hr = mel_hr.squeeze(0)  # [1, n_mels, T]
