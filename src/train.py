...
from src.models.audio_srgan import AudioSRGenerator  # Discriminator no longer used
...
def train(args):
    ...
    # -----------------------------------
    #  Mel extractor (Vocos-compatible)
    # -----------------------------------
    log_mel = LogMelExtractor(
        ...
        n_mels=args.n_mels,
        padding="center",
        apply_log=False,  # keep linear mel to match Vocos
    )
    ...
    # Optional initial generator checkpoint
    if args.g_ckpt and Path(args.g_ckpt).is_file():
        ...
        # Support both full state and plain state_dict
    ...
    # -----------------------------------
    #  Resume from ckpt_last.pt (full state)
    # -----------------------------------
    if args.resume:
        if args.resume_ckpt:
    ...
    # -----------------------------------
    #  Loss: BAE-like mel L1 + multi-res STFT on mel
    # -----------------------------------
    mel_loss = MelLoss(
        lambda_l1=1.0,
        lambda_l2=1.0,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for mel_lr, mel_hr in dataloader:
            mel_lr = mel_lr.to(device)  # [B, 1, n_mels, T]
            mel_hr = mel_hr.to(device)  # [B, 1, n_mels, T]

            # Some collate configs can add an extra dimension, keep it robust
            if mel_lr.dim() == 5:
            ...
            # BAE-like reconstruction loss (mel L1 + multi-res STFT on mel)
    ...
    # Final generator-only checkpoint for inference
...
def parse_args():
    ...
    # Optional initial generator weights
    parser.add_argument(
        "--g_ckpt",
        type=str,
        default="",
