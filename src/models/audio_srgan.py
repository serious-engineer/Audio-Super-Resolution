...
class ResidualBlock(nn.Module):
    """Residual block used in SRGAN-style generators."""
...
class AudioSRGenerator(nn.Module):
    ...
    def __init__(self, num_res_blocks: int = 16, num_features: int = 64):
        ...
        # No upsampling: we keep the same spatial size
...
class AudioSRDiscriminator(nn.Module):
    """Simple SRGAN-like discriminator on mel-spectrograms.

    Input:  [B, 1, n_mels, T] (log-mel)
